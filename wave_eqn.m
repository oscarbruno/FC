...
n = 50; % number of points in the initial domain
...
%               C, A, Q_tilde, Q_tilde, ArQ_tilder, AlQ_tildel, BC0, h) ...
%         - 59*fc_der(u(:, it - 1), der_coeffs2, filter_coeffs, d, C, ... 
%               A, Q_tilde, Q_tilde, ArQ_tilder, AlQ_tildel, BC1, h)...
%         + 37*fc_der(u(:, it - 2), der_coeffs2, filter_coeffs, d, C, ... 
%               A, Q_tilde, Q_tilde, ArQ_tilder, AlQ_tildel, BC2, h)...
%         - 9*fc_der(u(:, it - 3), der_coeffs2, filter_coeffs, d, C, ... 
%               A, Q_tilde, Q_tilde, ArQ_tilder, AlQ_tildel, BC3, h)); 
%     
...
%           Q_tilde, Q_tilde, ArQ_tilder, AlQ_tildel, BC, h);    
...
%               Q, Q_tilde, ArQ_tilder, AlQl, BC0, h) ...
%         - 59*fc_der(u(:, it - 1), der_coeffs2, filter_coeffs, d, C, ... 
%               A, Q, Q_tilde, ArQ_tilder, AlQl, BC1, h)...
%         + 37*fc_der(u(:, it - 2), der_coeffs2, filter_coeffs, d, C, ... 
%               A, Q, Q_tilde, ArQ_tilder, AlQl, BC2, h)...
%         - 9*fc_der(u(:, it - 3), der_coeffs2, filter_coeffs, d, C, A, ...
%               Q, Q_tilde, ArQ_tilder, AlQl, BC3, h));
%     u(:, it + 1) = u(:, it) + deltat/24*(55*g(:, it) 
%           - 59*g(:, it - 1) + 37*g(:, it - 2) - 9*g(:, it - 3));
%     BC = [0, 1; dxuexact(0, t), dxuexact(1, t + deltat)];
%     u(:, it + 1) = fc_der(u(:, it + 1), 1, filter_coeffs, d, C, A, ... 
%           Q, Q_tilde, ArQ_tilder, AlQl, BC, h);
...
%     Left and Right Dirichlet boundary conditions
    ...
    g(:, it + 1) = g(:, it) + deltat/24*(...
          55*fc_der(u(:, it), der_coeffs2, filter_coeffs, d, C, A, Q, ... 
                Q, ArQ_tilder, AlQl, BC0, h) ...
        - 59*fc_der(u(:, it - 1), der_coeffs2, filter_coeffs, d, C, A, ... 
                Q, Q, ArQ_tilder, AlQl, BC1, h)...
        + 37*fc_der(u(:, it - 2), der_coeffs2, filter_coeffs, d, C, A, ... 
                Q, Q, ArQ_tilder, AlQl, BC2, h)...
        - 9*fc_der(u(:, it - 3), der_coeffs2, filter_coeffs, d, C, A, ... 
                Q, Q, ArQ_tilder, AlQl, BC3, h));
    u(:, it + 1) = u(:, it) + deltat/24*(55*g(:, it) - ... 
            59*g(:, it - 1) + 37*g(:, it - 2) - 9*g(:, it - 3));
    u(1, it + 1) = uexact(0, t + deltat);
